          ...
          {assets.map((a) => (
            <Grid key={a.id} size={{ xs: 12, md: 6, lg: 4 }}>
              <Link href={`/assets/${a.id}`} style={{ textDecoration: "none" }}>
                <Card sx={{ height: "100%", borderRadius: 4, transition: "0.2s", "&:hover": { boxShadow: 6, transform: "translateY(-4px)" } }}>
                  <CardContent>
                    <Stack spacing={2}>
                      ...
                      <Stack direction="row" justifyContent="space-between">
                        ...
                        <Typography>¥{Number(a.total_value).toLocaleString()}</Typography>
                      </Stack>
                      <Stack direction="row" justifyContent="space-between">
                        <Typography color="text.secondary">Price / Share</Typography>
                        <Typography>¥{Number(a.price_per_token).toLocaleString()}</Typography>
                      </Stack>
                      <Stack direction="row" justifyContent="space-between">
